...
# os.environ['CUDA_VISIBLE_DEVICES'] = '0,1,2,3' # hsy11.19改
...
# Initialize network
net = highwayNet(args)
# net=torch.nn.parallel.DistributedDataParallel(net) # hsy11.19改
# net=torch.nn.DataParallel(net)# hsy11.19改
if args['use_cuda']:
    net = net.cuda()
...
#batch_size = 256 #hsy cslstm2里11.19
#batch_size = 32 #hsy cslstm3里 11.19
batch_size = 1024 #hsy cslstm4里 11.19
# batch_size = 128 #原
...
for epoch_num in range(pretrainEpochs+trainEpochs):
    ...
    for i, data in enumerate(trDataloader):
        ...
        # op_mask = op_mask.bool()  # hsy改10.24 为什么要mask这是干嘛的？？？
        mask = mask.bool()  # hsy改11.19
        ...
        if i%100 == 99:
            ...
            print("Epoch no:",epoch_num+1,"| Epoch progress(%):",format(i/(len(trSet)/batch_size)*100,'0.2f'), "| Avg train loss:",format(avg_tr_loss/100,'0.4f'),"| lat Acc/lon Acc:",format(avg_lat_acc,'0.4f'),format(avg_lon_acc,'0.4f'), "| Validation loss prev epoch",format(prev_val_loss,'0.4f'), "| ETA(s):",int(eta))
    ...
    print('Validation loss :',format(avg_val_loss/val_batch_count,'0.4f'),"| Val lat Acc/ Val lon Acc:",format(avg_val_lat_acc/val_batch_count*100,'0.4f'),format(avg_val_lon_acc/val_batch_count*100,'0.4f'))
    ...
    #1119 hsy改__________________________________________________________________________________________________________________________________________________________________________________________________________________________________________________________________________________________________________
if not os.path.isdir('./models'):
    os.mkdir('./models')
torch.save(net.state_dict(), './models/cslstm_m_1119.tar')
